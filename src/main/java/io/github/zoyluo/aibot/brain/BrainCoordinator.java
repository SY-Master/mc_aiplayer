...
public final class BrainCoordinator {
    ...
    static boolean shouldContinueAfterControl(ActionDispatcher.ControlEffect effect, boolean activeTask, boolean activeGoal, int queuedGoals, boolean activeAction) {
        return effect != ActionDispatcher.ControlEffect.NONE && hasRuntimeWork(activeTask, activeGoal, queuedGoals, activeAction);
    }

    static boolean hasRuntimeWork(boolean activeTask, boolean activeGoal, int queuedGoals, boolean activeAction) {
    ...
    private static void logStaleDecision(DecisionLease lease, String callback) {
        BotLog.commSystem("stale_decision_dropped", "bot_uuid", lease.botId(), "session_id", lease.sessionId(), "epoch", lease.epoch(), "request_sequence", lease.requestSequence(), "callback", callback);
    }
    ...
    private void onResponse(AIPlayerEntity bot, DecisionLease lease, ChatResponse response) {
        ...
        BotLog.api(bot, "api_response", "tokens_in", response.promptTokens(), "tokens_out", response.completionTokens(), "cache_hit", response.promptCacheHitTokens(), "finish_reason", response.finishReason());
    ...
    public BrainStatus status(AIPlayerEntity bot) {
        ...
        return new BrainStatus(conversation.decision.busy(), conversation.history.size(), conversation.lastPromptTokens, conversation.lastCompletionTokens, conversation.lastCacheHitTokens);
    }
    ...
    private void submit(AIPlayerEntity bot, BotConversation conversation, DecisionLease lease) {
        try {
            ...
            List<ToolDefinition> toolsSnapshot = toolRegistry.tools(brainConfig, brainConfig.exposesLowLevelTools() || manualMode(bot), BotRuntimeOptions.INSTANCE.memoryToolsEnabled(bot), brainConfig.coordinationToolsEnabled());
            executor.submit(bot, lease, historySnapshot, toolsSnapshot, (responseLease, response) -> onResponse(bot, responseLease, response), (errorLease, throwable) -> onError(bot, errorLease, throwable));
    ...
    public record BrainStatus(boolean busy, int historySize, int promptTokens, int completionTokens,
                              int cacheHitTokens) {
    }
}
