...
/**
 * 单个 bot 的危险应对策略(LLM 经 set_danger_policy 工具设置,持久化到世界存档)。
...
public record DangerPolicy(Mode mode, Integer retreatHp, Integer maxEnemies, Boolean keepSurvival) {
    ...
    public static final DangerPolicy DEFAULT = new DangerPolicy(null, null, null, null);
    ...
    /** 与默认等价(mode 空/AUTO 且无覆盖)→ 不必持有/持久化,存储层据此删条目。 */
